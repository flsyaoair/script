...
def multiConfigNexus():
    upFile=open('releases.txt','r')
    upFileContent=upFile.readlines()
    upFile.close()
    
    for i in upFileContent:
        upFileName=i.strip('\n')
        pomPath='.\\'+upFileName+'\\*.*'
        goalPath='..\\package2'+'\\'
        print pomPath
        subprocess.check_call('xcopy /Y /e  %s %s' %(pomPath,goalPath)) 
#        subprocess.check_call('devenv.com ..\workstateServer.sln /build RELEASE /out ..\output.txt')
#        pName=upFileName.rsplit('.')
        print upFileName
#    groupId=hudsonBuild().upload(moduleName1,moduleName2,moduleName3) 
#    global JOB_NAME
#    global WORKSPACE
